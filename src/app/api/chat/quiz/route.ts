  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        content_type: params.contentType || 'quiz', // Default to 'quiz' if not specified
      })
    });
...
function addSystemPromptWithLatex(messages: ChatMessage[], mode: string): ChatMessage[] {
  ...
  // Get quiz mode system prompt
  let systemPrompt = `You are a quiz assistant that helps students learn through testing their knowledge.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Add system message for quiz mode with LaTeX support
    const messages = addSystemPromptWithLatex(trimmedMessages, 'quiz');
    ...
    if (KEYWORDS_AI_API_KEY) {
      ...
      logToKeywordsAI({
        ...
        generationTime,
        contentType: 'quiz' // Specify content type as 'quiz'
      });
    }
    ...
    if (KEYWORDS_AI_API_KEY) {
      logToKeywordsAI({
        ...
        contentType: 'quiz-error' // Use specific content type for errors
      });
    }
